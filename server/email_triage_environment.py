...
# - Each task is a SINGLE-STEP episode: reset(task_id) → step(action) → done=True
# - Returns external reward clamped to (0.01, 0.99)
...
class EmailTriageEnvironment:
    ...
    def reset(self, phase: int = 1, seed: Optional[int] = None) -> Dict[str, Any]:
        """Reset for a specific task phase. Each task is a single-step episode."""
        if seed is not None:
        ...
        # Set the task_id directly from phase — no auto-advancing
    ...
    def step(self, action: TriageAction) -> Dict[str, Any]:
        """Evaluate the action and end the episode immediately (single-step)."""
        ...
        if not valid:
            ...
            return self._make_observation(reward=external_reward, raw_reward=raw_reward, done=True, feedback=feedback)
        ...
        # --- Priority & Department scoring (task 2 and 3) ---
        if task_id >= 2:
        ...
        # --- Draft reply scoring (task 3 only) ---
        if task_id == 3:
            ...
            if word_count < 100:
                ...
                if sentences:
                    ...
                    rep_penalty = min(0.15, repetition_ratio * 0.4)
                    if rep_penalty > 0:
                        reward -= rep_penalty
                        feedback_items.append(f"Repetition penalty ({repetition_ratio:.2f})")
                ...
                if gt.get("disguised"):
                    ...
                    reward += min(0.15, hits * 0.04)
                    feedback_items.append(f"Phishing awareness keywords: {hits}")
    ...
    def _make_observation(self, reward: float, raw_reward: float, done: bool, feedback: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "sender": e.get("sender", ""),
            "task_id": task_id,
            "task_description": f"TASK {task_id}: " + {1: "Classify email", 2: "Classify, prioritize, and route", 3: "Full triage with reply"}.get(task_id, "Unknown"),
            "reward": reward,
            "raw_reward": raw_reward,
            "done": done,
            "feedback": feedback,
